...
# Load data
data = pd.read_csv(PROJECT_DIR / 'processed_data' / 'yield_data' / 'swiss_spot_yields.csv')
...
# data = data.dropna(thresh=len(data) - 1000, axis=1) #for UK
...
X, Y = np.meshgrid(X_numeric, np.arange(len(data.columns)))
...
# Save figure
plt.savefig('figures/yield_curves/snb_yield_curve.png', format='png', dpi=300)
