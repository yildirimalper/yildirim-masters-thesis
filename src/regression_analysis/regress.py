...
ecb_results = sm.OLS(ecb["10yr Change"], sm.add_constant(ecb[["In ECB 3dWindow", "In Fed 3dWindow"]])).fit()
...
boe_results = sm.OLS(boe["10yr Change"], sm.add_constant(boe[["In BoE 3dWindow", "In Fed 3dWindow"]])).fit()
...
boj_results = sm.OLS(boj["10yr Change"], sm.add_constant(boj[["In BoJ 3dWindow", "In Fed 3dWindow"]])).fit()
...
snb_results = sm.OLS(snb["10yr Change"], sm.add_constant(snb[["In SNB 3dWindow", "In Fed 3dWindow"]])).fit()
...
rba_results = sm.OLS(rba["10yr Change"], sm.add_constant(rba[["In RBA 3dWindow", "In Fed 3dWindow"]])).fit()
...
boe_results = sm.OLS(boc["10yr Change"], sm.add_constant(boc[["In BoC 3dWindow", "In Fed 3dWindow"]])).fit()
boe_stargazer = Stargazer([boe_results])
...
# without BoC here
stargazer.custom_columns(['ECB', 'BoE', 'BoJ', 'SNB', 'RBA'], [1, 1, 1, 1, 1])
